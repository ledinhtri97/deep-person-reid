...
def pairwise_distances(embeddings, metric: str = "euclidean"):
...
def triplet_loss(distances, mask_pos, mask_neg, margin: float):
...
class OctupletLoss(torch.nn.modules.loss._Loss):
    def __init__(self, use_gpu=True, margin: float = 0.5, metric: str = "euclidean", configuration: list = None):
