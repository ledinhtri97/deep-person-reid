...
try:
    ...
    def selective_scan_fn(x, dt, A,  B,  C, k, **kwargs): return 0
...
def mamba_vision_T(num_classes, loss='softmax', pretrained=True, **kwargs):
...
def mamba_vision_T2(num_classes, loss='softmax', pretrained=True, **kwargs):
...
def mamba_vision_S(num_classes, loss='softmax', pretrained=True, **kwargs):
    ...
    # model.pretrained_cfg = pretrained_cfg
    # model.default_cfg = model.pretrained_cfg
    model.default_cfg = default_cfgs['mamba_vision_S']
    if pretrained:
        if not Path(model_path).is_file():
            url = model.default_cfg['url']
            torch.hub.download_url_to_file(url=url, dst=model_path)
...
def mamba_vision_B(num_classes, loss='softmax', pretrained=True, **kwargs):
...
def mamba_vision_L(num_classes, loss='softmax', pretrained=True, **kwargs):
...
def mamba_vision_L2(num_classes, loss='softmax', pretrained=True, **kwargs):
